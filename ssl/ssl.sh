#! /bin/sh
set -uo errexit
alias kdevel='KUBECONFIG=$HOME/.kube/ccloud-config/devel/kubeconfig kubectl'
export APP="${1}"
export NAMESPACE="${2}"
...
echo "kdevel delete csr ${CSR_NAME} || :"
kdevel delete csr ${CSR_NAME} || :

echo "... creating kubernetes CSR object"
echo "kdevel create -f -"
kdevel create -f - <<EOF
...
while true; do
  ...
  echo "kdevel get csr ${CSR_NAME}"
  kdevel get csr ${CSR_NAME} > /dev/null 2>&1
  if [ "$?" -eq 0 ]; then
      break
  fi
...
kdevel certificate approve ${CSR_NAME}

SECONDS=0
while true; do
  echo "... waiting for serverCert to be present in kubernetes"
  echo "kdevel get csr ${CSR_NAME} -o jsonpath='{.status.certificate}'"
  serverCert=$(kdevel get csr ${CSR_NAME} -o jsonpath='{.status.certificate}')
  if [[ $serverCert != "" ]]; then
    break
  fi
  if [[ $SECONDS -ge 60 ]]; then
